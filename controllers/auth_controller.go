...
func Register(c *gin.Context) {
	...
	user.Nickname = input.Nickname // 保存昵称
...
// ChangePasswordInput 定义修改密码的输入参数
...
// @Accept json
// @Produce json
// @Param body body ChangePasswordInput true "修改密码信息"
// @Success 200 {object} ApiResult[any] "Password changed successfully"
// @Failure 400 {object} ApiResult[string] "Invalid input"
// @Failure 401 {object} ApiResult[string] "Invalid old password"
// @Failure 500 {object} ApiResult[string] "Failed to change password"
...
func ChangePassword(c *gin.Context) {
	...
	if err := bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(input.OldPassword)); err != nil {
		c.JSON(http.StatusUnauthorized, ApiResult[any]{Status: "error", Code: 401, Message: "Invalid old password"})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, ApiResult[any]{Status: "error", Code: 500, Message: "Failed to encrypt new password"})
		return
	}
	...
	if err := config.DB.Save(&user).Error; err != nil {
		c.JSON(http.StatusInternalServerError, ApiResult[any]{Status: "error", Code: 500, Message: "Failed to change password"})
		return
	}

	c.JSON(http.StatusOK, ApiResult[any]{Status: "success", Code: 200, Message: "Password changed successfully"})
}
